...
// file:      OGLAnimRotation.h
//
// summary:   Declares the ogl animation rotation class
//
...
#if !defined(AFX_OGLANIMROTATION_H__INCLUDED_)
#define AFX_OGLANIMROTATION_H__INCLUDED_
...
class CLUDRAW_API COGLAnimRotation  : public COGLRotation
{
public:
	enum EAnimMode
	{
...
public:
	COGLAnimRotation();
	COGLAnimRotation( float fAngle, float fX, float fY, float fZ );
	COGLAnimRotation(const COGLAnimRotation& rRot);

	virtual COGLBaseElement* Copy()
	{
		return (COGLBaseElement *) new COGLAnimRotation( *this );
	}

	COGLAnimRotation& operator= (const COGLAnimRotation& rRot);
	...
	//void SetLocalTime( double dTime ) { m_dLocalTime = dTime; }
	//void EnableLocalTime( bool bVal = true ) { m_bUseLocalTime = bVal; }
...
protected:
	...
	//// Use local time. Time only advances when apply is executed.
