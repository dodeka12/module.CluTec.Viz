...
// file:      CLUOutput.cpp
//
// summary:   Implements the clu output class
//
...
CCLUOutput::CCLUOutput(int iPosX, int iPosY, int iWidth, int iHeight, const char* pcTitle, void* pvIcon)
{
	m_poglWin     = 0;
	m_pEditor     = 0;
	m_iFontSize   = 14;
...
void CCLUOutput::Copy()
{
	...
	Fl::copy(sText.Str(), int(sText.Len()), 1);
}
...
const char* CCLUOutput::InfoLink(Fl_Widget* pWidget, const char* pcURL)
{
	...
	else if (strstr(pcURL, "clu_output://"))
	{
		...
		strncpy_s(pcCmd, 30, &(pcURL[13]), 29);
		...
		*pcSlash = 0;
		...
		strncpy_s(pcVal, 30, &(pcSlash[1]), 29);
		...
		*pcSlash = 0;
		...
		{
			int iObjCnt = int(m_vecOutObj.size());
...
{
	int iObj, iObjCnt = int(rvecOutObj.size());
...
/////////////////////////////////////////////////////////////////////////////
void CCLUOutput::Clear()
{
	if (m_vecOutObj.size() > 0)
	{
		m_vecOutObj.clear();
		Update();
	}
...
bool CCLUOutput::GenHTML(CStrMem& sText, bool bGenLinks, bool bGenFontCmd)
{
	int iCurSize;
	int iObj, iObjCnt = int(m_vecOutObj.size());
