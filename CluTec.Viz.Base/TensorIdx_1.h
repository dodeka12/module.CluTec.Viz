...
// file:      TensorIdx_1.h
//
// summary:   Declares the tensor index 1 class
//
...
public:
	struct SFreeIdxData
	{
		int iIdx;			// Current index value. Tensor index is mRange[iIdx].
		int iStep;			// Number of elements in tensor to step over
							// if index changes by one.
		Mem<int> mRange;	// Selection of indices for free index.
							// Number of elements of this list gives
...
public:
	...
	void SetIdxList(int iIdx, Mem<int> mIdx) throw (CCluException);
	...
	// Step Count. Get number of steps in given loop.
