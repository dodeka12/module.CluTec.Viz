...
// file:      Macopt.cpp
//
// summary:   Implements the macopt class
//
...
bool MacOpt::MacOptII( double *p )                   
{
	...
	*/
	...
	for ( a_its = 0 ; a_its < a_itmax ; a_its ++ ) 
	{
		...
		if ( a_verbose > 0 ) 
		{
			sprintf_s(hstr, 200, "mac_it %d of %d : gg = %6.3g tol = %6.3g: ", a_its , a_itmax , gg , a_tol );
		...
		if ( a_restart == 0 ) 
		{
			if ( a_verbose > 1 ) 
			{
				sprintf_s(hstr, 200, "(step %9.5g)",step);
		...
		else 
		{
			...
			{
				sprintf_s(hstr, 200, "New line search has inner prod %9.4g", tmpd ); 
...
double MacOpt::MacLinMinII(double *p)
{
	...
	{ /* check this is true: (no need to do this really
	  as it is already checked at the end of the main
		...
		*/
		sprintf_s (hstr, 200, "inner product at 0 = %9.4g", tmpd = MacProdII ( p , gy , 0.0 ) ) ; 
		msgList.SetMessage(MSG_VB3, hstr);
	...
	{  /* we need to go further */
		do 
		{
			...
			if ( a_verbose > 1 ) 
			{
				sprintf_s(hstr, 200, "s = %6.3g: t = %6.3g; x = %6.3g y = %6.3g",s, t , x , y );
				msgList.SetMessage(MSG_VB2, hstr);
			}
	...
	if ( its >= a_linmin_maxits )  
	{
		...
		*/
		...
		sprintf_s(hstr, 200, "- inner product at 0 = %9.4g" ,
...
they agree for a step of size epsilon */
{
	...
	for ( j = 0 ; j < stopat ; j ++ ) 
	{
		...
		sprintf_s(hstr, 200, "%2d %9.5g %9.5g %9.5g" , j , g[j] , h[j]/epsilon , g[j] - h[j]/epsilon );
