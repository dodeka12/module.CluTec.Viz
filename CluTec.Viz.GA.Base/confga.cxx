...
void ConfGA<CType>::InitVars()
{
	...
	vE3[iE3] = 1;
	vEp.SetStyle(m_MVStyle);
	vEp[iEp] = 1;
	vEm.SetStyle(m_MVStyle);
	vEm[iEm] = 1;

	vE.SetStyle(m_MVStyle);
	vE[iEp] = CType(1);
	vE[iEm] = CType(1);						// e = e+ + e-
	vE0.SetStyle(m_MVStyle);
	vE0[iEm] = CType(0.5);
	vE0[iEp] = CType(-0.5);							// e0 = 0.5 (e- - e+)

	vN.SetStyle(m_MVStyle);
	vN = vE;			// n = e = e+ + e-
	vNb.SetStyle(m_MVStyle);
	vNb = vE0;				// nb = e0 = 0.5 (e- - e+)

	vB1.SetStyle(m_MVStyle);
	vB1[iB1] = 1;
	vB2.SetStyle(m_MVStyle);
	vB2[iB2] = 1;
	vB3.SetStyle(m_MVStyle);
	vB3[iB3] = 1;
	vBp1.SetStyle(m_MVStyle);
	vBp1[iBp1] = 1;
	vBp2.SetStyle(m_MVStyle);
	vBp2[iBp2] = 1;
	vBp3.SetStyle(m_MVStyle);
	vBp3[iBp3] = 1;
	vBm1.SetStyle(m_MVStyle);
	vBm1[iBm1] = 1;
	vBm2.SetStyle(m_MVStyle);
	vBm2[iBm2] = 1;
	vBm3.SetStyle(m_MVStyle);
	vBm3[iBm3] = 1;
	vBpm.SetStyle(m_MVStyle);
	vBpm[iBpm] = 1;

	vTp1.SetStyle(m_MVStyle);
	vTp1[iTp1] = 1;
	vTp2.SetStyle(m_MVStyle);
	vTp2[iTp2] = 1;
	vTp3.SetStyle(m_MVStyle);
	vTp3[iTp3] = 1;
	vTm1.SetStyle(m_MVStyle);
	vTm1[iTm1] = 1;
	vTm2.SetStyle(m_MVStyle);
	vTm2[iTm2] = 1;
	vTm3.SetStyle(m_MVStyle);
	vTm3[iTm3] = 1;
	vTpm1.SetStyle(m_MVStyle);
	vTpm1[iTpm1] = 1;
	vTpm2.SetStyle(m_MVStyle);
	vTpm2[iTpm2] = 1;
	vTpm3.SetStyle(m_MVStyle);
	vTpm3[iTpm3] = 1;
	vT123.SetStyle(m_MVStyle);
	vT123[iT123] = 1;

	vQp.SetStyle(m_MVStyle);
	vQp[iQp] = 1;
	vQm.SetStyle(m_MVStyle);
	vQm[iQm] = 1;
	vQpm1.SetStyle(m_MVStyle);
	vQpm1[iQpm1] = 1;
	vQpm2.SetStyle(m_MVStyle);
	vQpm2[iQpm2] = 1;
	vQpm3.SetStyle(m_MVStyle);
	vQpm3[iQpm3] = 1;
...
template<class CType>
MultiV<CType> ConfGA<CType>::Rotor(const CType& a1, const CType& a2, const CType& a3, const CType& rad, CType fPrec)
{
	MultiV<CType> vA(m_MVStyle);
...
template<class CType>
MultiV<CType> ConfGA<CType>::Rotor(const MultiV<CType>& vX, const CType& rad, bool bAxis, CType fPrec)
{
	MultiV<CType> vA(m_MVStyle);
...
template<class CType>
CStrMem ConfGA<CType>::MakeStrVector(const MultiV<CType>& vA)
{
	CStrMem csVec;
	CType dE, dE0, dVal;
...
template<class CType>
bool ConfGA<CType>::AnalyzeMV(const MultiV<CType>& vX, CMVInfo<CType>& rMVI, bool bOPNS, CType fPrec)
{
	MultiV<CType> vA(vX);
	Mem<uint> mGList;
	int iGrades;
	...
	if (((iGrades = int(mGList.Count())) == 0) || (iGrades > 3)) { return false; }
...
template<class CType>
bool ConfGA<CType>::AnalyzeBlade(const MultiV<CType>& vA, CMVInfo<CType>& rMVI, bool bOPNS, CType fPrec)
{
