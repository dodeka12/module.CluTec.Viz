...
// Generates all blades of a particular grade, using recursion.
	template<class CType>
	bool El2GA<CType>::GenerateBlades(BladeList<CType>& blList, MemObj<Mem<uint> >& mBladeIdx, uint uGrade, uint uFirstIdx, uint* puIdxList, uint uIdxPos)
	{
...
/// Init Variables
	...
	void El2GA<CType>::InitVars()
	{
		...
		vE3[3] = 1;
		vE4.SetStyle(m_MVStyle);
		vE4[4] = 1;
		vE5.SetStyle(m_MVStyle);
		vE5[5] = 1;
		vE6.SetStyle(m_MVStyle);
		vE6[6] = 1;
...
// of the given conics.
	...
	template<class CType>
	bool El2GA<CType>::FindAllDegConics(const MultiV<CType>& vA, const MultiV<CType>& vB, MemObj<MultiV<CType> >& mConicList, CType fPrec)
	{
		if ((vA.BasePtr()->BaseID() != ID_EL2GA) ||
...
// Expects vA and vB to represent conics as vectors, i.e. through their IPNS.
	...
	template<class CType>
	bool El2GA<CType>::IntersectConics(const MultiV<CType>& vA, const MultiV<CType>& vB, MemObj<CPointND<CType, 2> >& mPointList, CType fPrec)
	{
		if ((vA.BasePtr()->BaseID() != ID_EL2GA) ||
		    (vB.BasePtr()->BaseID() != ID_EL2GA))
		{
			...
			if (Info.m_eType == GA_LINEPAIR)
			{
				...
				for (j = 0; j < 2; j++)
				{
					...
					if (dSr > dZeroMax)
					{
						...
						size_t iCount = mPointList.Count();
						mPointList.Add(2);
...
// If function returns false, MV could not be analyzed.
	...
	template<class CType>
	bool El2GA<CType>::AnalyzeMV(const MultiV<CType>& vX, CMVInfo<CType>& rMVI, bool bOPNS, CType fPrec)
	{
		MultiV<CType> vA(vX);
		Mem<uint> mGList;
		size_t iGrades;
...
// If function returns false, MV could not be made into degenerate conic
	...
	template<class CType>
	bool El2GA<CType>::MakeConicDegenerate(const MultiV<CType>& vA, MultiV<CType>& vB, CType& fDist, bool bOPNS, CType fPrec)
	{
		MultiV<CType> vX(vA);
...
// If function returns false, MV could not be analyzed.
	...
	template<class CType>
	bool El2GA<CType>::AnalyzeBlade(const MultiV<CType>& vA, CMVInfo<CType>& rMVI, bool bOPNS, CType fPrec)
	{
		MultiV<CType> vX(vA);
		MemObj<MultiV<CType> > mvList;
		CType fZeroMin, fZeroMax, fUnityMin, fUnityMax;
...
*/
		...
		else if (iGrade == 2)
		{
			...
			size_t i, iCount = mPointList.Count();
