...
// file:      multiv.h
//
// summary:   Declares the multiv class
//
...
//#ifdef _GNUCPP3_
	...
	template<class CType> bool FactorBlade(const MultiV<CType>& mvA,           MemObj<MultiV<CType> >& mvList, bool bNorm, MultiV<CType>* pvB, CType* pFac, CType dPrec);

	template<class CType> bool FactorBlade(const MultiV<CType>& mvA, const MultiV<CType>& vNegDim, MemObj<MultiV<CType> >& mvList, bool bNorm, MultiV<CType>* pvB, CType* pFac, CType dPrec);

	template<class CType> bool FactorBlade(const MultiV<CType>& vA,            MemObj<MultiV<CType> >& mvList, MemObj<MultiV<CType> >& mvBasis, bool bNorm, int* piOrder, CType* pFac, CType dPrec);
...
// Multivector Product Matrices
	...
	template<class CType> bool CalcMultiVGPMat(const MultiV<CType>& mvA, Matrix<CType>& M, tMVPos bPos = Right, bool bEven = false);

	template<class CType>  bool CalcMultiVGPMat(const MultiV<CType>& mvA, Matrix<CType>& M, tMVPos bPos, const Mem<int>& mGrades);

	template<class CType>  bool CalcMultiVGPMatMask(const MultiV<CType>& mvA, Matrix<CType>& M, tMVPos bPos, const Mem<int>& mMask, EMVOpType eProdType = MVOP_GEO);

	template<class CType>  bool CalcMVListGPMatMask(const MemObj<MultiV<CType> >& mvAList,
		Matrix<CType>& M,
		tMVPos bPos,
		const Mem<int>& mMask,
		EMVOpType eProdType = MVOP_GEO,
...
// Cab: Cross-Covariance Matrix of A and B
	...
	template<class CType>  bool EvalEPMeanProd(MultiV<CType>& vC, EMVOpType eProdType,     const MultiV<CType>& vA, const MultiV<CType>& vB, const Matrix<CType>& Cab);
...
// Cxy: Cross-Covariance Matrix of X and Y
	...
	template<class CType>  bool EvalEPCovMatProd(Matrix<CType>& Cuu,
		EMVOpType eProdType,
		const MultiV<CType>& vX,
		const MultiV<CType>& vY,
		const Matrix<CType>& Cxx,
		const Matrix<CType>& Cyy,
...
// Cxz, Cyz: Cross-Covariance Matrices of X and Y with Z
	...
	template<class CType>  bool EvalEPCrossCovMatProd(Matrix<CType>& Cuz, EMVOpType eProdType, const MultiV<CType>& vX, const MultiV<CType>& vY, const Matrix<CType>& Cxz,
		const Matrix<CType>& Cyz);

	template<class CType>  bool MultiVSVD(const MultiV<CType>& mv, MemObj<MultiV<CType> >& mvList, Mem<CType>& evList, tMVPos bPos = Right);

	template<class CType>  uint MultiVJacobi(const MultiV<CType>& mv, MemObj<MultiV<CType> >& mvEVecList, Mem<CType>& mEValList, tMVPos bPos = Right);

	template<class CType>  bool MultiVSolve(const MultiV<CType>& mvA, const MultiV<CType>& mvB, MultiV<CType>& mvX, tMVPos bPos = Left, Mem<CType>* pmDiag = 0);

	template<class CType>  bool MultiVGEVSolve(const MultiV<CType>& mvA, const MultiV<CType>& mvB, MemObj<MultiV<CType> >& mvX, tMVPos bPos1 = Left, tMVPos bPos2 = Left, bool bEven = false);

	template<class CType> MultiV<CType> Join(const MultiV<CType>& vA, const MultiV<CType>& vB, bool bNorm = true, CType dPrec = 0);

	template<class CType> MultiV<CType> Join(const MultiV<CType>& vA, const MultiV<CType>& vB, const MultiV<CType>& vNegDim, bool bNorm = true, CType dPrec = 0);
...
*/
	...
	template<class CType> bool MultiVGEVSolveMask(const MultiV<CType>& vA,
		const MultiV<CType>& vB,
	...
	template<class CType> bool MultiVGEVSolveMask(const MemObj<MultiV<CType> >& vA,
		const MemObj<MultiV<CType> >& vB,
		MemObj<MultiV<CType> >& mvXList,
		Mem<CType>& mDiag,
		tMVPos bPos1,
		tMVPos bPos2,
	...
	template<class CType> bool EvalVersor(const MemObj<MultiV<CType> >& vA, const MemObj<MultiV<CType> >& vB, MemObj<MultiV<CType> >& mvXList, Mem<CType>& mDiag, const Mem<int>& mMask);

	template<class CType> bool MultiVGEVSolve(const MultiV<CType>& vA, const MultiV<CType>& vB, const MultiV<CType>& vC, MultiV<CType>& vX, tMVPos bPos1, tMVPos bPos2, const Mem<int>& mGrades);
	template<class CType> bool MultiVGEVSolveMask(const MultiV<CType>& vA, const MultiV<CType>& vB, const MultiV<CType>& vC, MultiV<CType>& vX, tMVPos bPos1, tMVPos bPos2, const Mem<int>& mMask);
...
// Calculate Join/Meet for Blades.
	...
	template<class CType> MultiV<CType> Meet(const MultiV<CType>& mvA, const MultiV<CType>& mvB, const MultiV<CType>& vNegDim, CType dPrec = 0);
...
// Multi Vector Class Declaration ----------------------------
	...
	public:
		...
		bool MTProd(const Mem<CType>& mA, const Mem<CType>& mB, short* piTable);
/*
