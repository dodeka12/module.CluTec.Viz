...
#include "CluTec.Viz.Base\matrix.cxx"
...
CType MultiV<CType>::Mag2() const
{
	...
	short* viTable = m_pStyle->VecInvTable();
...
MultiV<CType> MultiV<CType>::operator()(uint grade) const
{
	...
	short* grades = m_pStyle->Grades();

	for (uint i = 0; i < m_uGADim; i++)
	{
		if ((uint) grades[i] == grade)
		{
...
{
	short* grades = a.m_pStyle->Grades();
...
MultiV<CType> Involute(const MultiV<CType>& vA)
{
	...
	short* invoTable = vB.m_pStyle->InvoTable();
...
MultiV<CType> operator~(const MultiV<CType>& amv)
{
	...
	short* revTable = cmv.m_pStyle->ReverseTable();
...
MultiV<CType> operator*(const MultiV<CType>& amv)
{
	...
	int r;
	short* dualTable = cmv.m_pStyle->DualTable();
...
{
	uint i;
	short* grades = amv.m_pStyle->Grades();
	MultiV<CType> cmv(amv.m_pStyle[0]);
	cmv = amv;
...
MultiV<CType> operator+(const MultiV<CType>& amv, const Blade<CType>& b)
{
	...
	if ((r = (amv.m_pStyle[0] == b)))
	{
		...
		cmv.m_mData[r] += b.m_xFact * CType(f);
	}
...
MultiV<CType>& operator+=(MultiV<CType>& amv, const Blade<CType>& b)
{
	...
	if ((r = (amv.m_pStyle[0] == b)))
	{
		...
		amv.m_mData[r] += b.m_xFact * CType(f);
	}
...
template<class CType>
bool MultiV<CType>::MTProd(const Mem<CType>& mA, const Mem<CType>& mB, short* piTable)
{
	int r;
	uint i, j, k;
	uint uADim, uBDim;
...
MultiV<CType> operator&(const MultiV<CType>& a, const MultiV<CType>& b)
{
	short* piTable = a.m_pStyle->GPTable();
	MultiV<CType> c(a.m_pStyle[0]);
...
MultiV<CType> operator*(const MultiV<CType>& a, const MultiV<CType>& b)
{
	short* piTable = a.m_pStyle->IPTable();
	MultiV<CType> c(a.m_pStyle[0]);
...
MultiV<CType> operator^(const MultiV<CType>& a, const MultiV<CType>& b)
{
	short* piTable = a.m_pStyle->OPTable();
	MultiV<CType> c(a.m_pStyle[0]);
		...
		for (i = 0; i < m_uGADim; i++)
		{
			if (!(m_mData[i] == CType(0)))
			{
				...
				if (i > 0)
				{
					...
					if ((j = uint('^' < hstr)))
					{
						m_csStr << hstr.Last(int(hstr.Len() - j));
					}
				}
				m_csStr << " ";
			}
...
{
	uint i, j;
	short* grades = mv.m_pStyle->Grades();
...
bool GradeList(const MultiV<CType>& vA, Mem<uint>& mList, CType dPrec)
{
	...
	short* grades = vA.GetStyle().Grades();
	...
	uint uCount = 0;
	for (uint i = m_uGADim; i > 0; i--)
	{
		if (!::IsZero(vA.m_mData[i - 1], dPrec))
		{
			uint g = (uint) grades[i - 1];
...
bool MultiV<CType>::GetMaxHMV(MultiV<CType>& vHMV, CType dPrec) const
{
	...
	if ((iGradeCount = int(mGList.Count())) == 1)
	{
		vHMV = vA;
	}
...
template<class CType>
bool FactorBlade(const MultiV<CType>& vA, const MultiV<CType>& vNegDim, MemObj<MultiV<CType> >& mvList, bool bNorm, MultiV<CType>* pvB, CType* pFac, CType dPrec)
{
...
template<class CType>
bool FactorBlade(const MultiV<CType>& vA, MemObj<MultiV<CType> >& mvList, bool bNorm, MultiV<CType>* pvB, CType* pFac, CType dPrec)
{
	int iGrade;
	CType dVal, dMax;
	Mem<uint> mGList;
...
template<class CType>
bool FactorBlade(const MultiV<CType>& vA, MemObj<MultiV<CType> >& mvList, MemObj<MultiV<CType> >& mvBasis, bool bNorm, int* piOrder, CType* pFac, CType dPrec)
{
	Mem<uint> mGList;
	uint uBaseVecCnt = uint(mvBasis.Count());
...
template<class CType>
MultiV<CType> Join(const MultiV<CType>& vA, const MultiV<CType>& vB, bool bNorm, CType dPrec)
{
...
template<class CType>
MultiV<CType> Join(const MultiV<CType>& vABlade, const MultiV<CType>& vBBlade, const MultiV<CType>& vNegDim, bool bNorm, CType dPrec)
{
	    ...
	    (mGList[0] != 0))
	{
		...
		iAVecCount = iAVecUsedCount = uint(mvAList.Count());
...
{
	uint m_uGADim  = vA.GetStyle().GADim();
	short* gpTable = vA.GetStyle().GPTable();
	short* grades  = vA.GetStyle().Grades();
...
bool CalcMultiVGPMat(const MultiV<CType>& vA, Matrix<CType>& M, tMVPos bPos, const Mem<int>& mGrades)
{
	...
	if (!(nGradeCount = uint(mGrades.Count()))) { return false; }
...
template<class CType>
bool CalcMultiVGPMatMask(const MultiV<CType>& vA, Matrix<CType>& M, tMVPos bPos, const Mem<int>& mMask, EMVOpType eOPType)
{
	uint m_uGADim = vA.GetStyle().GADim();
	short* gpTable;
...
template<class CType>
bool CalcMVListGPMatMask(const MemObj<MultiV<CType> >& mvAList, Matrix<CType>& M, tMVPos bPos, const Mem<int>& mMask, EMVOpType eOPType, bool bPruneNullRows)
{
...
template<class CType>
bool MultiVSVD(const MultiV<CType>& mvA, MemObj<MultiV<CType> >& mvList, Mem<CType>& evList, tMVPos bPos)
{
	uint i, j;
...
template<class CType>
uint MultiVJacobi(const MultiV<CType>& vA, MemObj<MultiV<CType> >& mvEVecList, Mem<CType>& mEValList, tMVPos bPos)
{
	uint i, j, roots;
...
template<class CType>
bool MultiVSolve(const MultiV<CType>& vA, const MultiV<CType>& vB, MultiV<CType>& vX, tMVPos bPos, Mem<CType>* pmDiag)
{
	uint i;
	uint m_uGADim = vA.m_pStyle->GADim();
...
template<class CType>
bool MultiVGEVSolve(const MultiV<CType>& vA, const MultiV<CType>& vB, MemObj<MultiV<CType> >& mvXList, tMVPos bPos1, tMVPos bPos2, bool bEven)
{
	uint i, j, row, n;
	uint m_uGADim = vA.GetStyle().GADim();
	short* grades = vA.GetStyle().Grades();
...
template<class CType>
bool MultiVGEVSolveMask(const MultiV<CType>& vA, const MultiV<CType>& vB, MemObj<MultiV<CType> >& mvXList, Mem<CType>& mDiag, tMVPos bPos1, tMVPos bPos2, const Mem<int>& mMask)
{
	uint i, j, row, n;
	uint m_uGADim = vA.GetStyle().GADim();
...
template<class CType>
bool MultiVGEVSolveMask(const MemObj<MultiV<CType> >& mvAList,
		const MemObj<MultiV<CType> >& mvBList,
		MemObj<MultiV<CType> >& mvXList,
		Mem<CType>& mDiag,
		tMVPos bPos1,
		tMVPos bPos2,
		const Mem<int>& mMask)
...
template<class CType>
bool EvalVersor(const MemObj<MultiV<CType> >& mvAList, const MemObj<MultiV<CType> >& mvBList, MemObj<MultiV<CType> >& mvXList, Mem<CType>& mDiag, const Mem<int>& mMask)
{
	uint i, j, row, n, iPos, rowcnt, col, colcnt, ActRow;
	uint m_uGADim = mvAList[0].GetStyle().GADim();
...
template<class CType>
bool MultiVGEVSolve(const MultiV<CType>& vA, const MultiV<CType>& vB, const MultiV<CType>& vC, MultiV<CType>& vX, tMVPos bPos1, tMVPos bPos2, const Mem<int>& mGrades)
{
	uint i, j, row, n;
	uint m_uGADim = vA.GetStyle().GADim();
	short* grades = vA.GetStyle().Grades();
	...
	n = uint(mGrades.Count());
	vX.SetStyle(*vA.m_pStyle);
	for (i = 0, row = 1; i < m_uGADim; i++)
	{
		for (j = 0; j < n; j++)
		{
			if ((int) grades[i] == mGrades[j])
			{
...
template<class CType>
bool MultiVGEVSolveMask(const MultiV<CType>& vA, const MultiV<CType>& vB, const MultiV<CType>& vC, MultiV<CType>& vX, tMVPos bPos1, tMVPos bPos2, const Mem<int>& mMask)
{
	uint i, row;
	uint m_uGADim = vA.GetStyle().GADim();
...
template<class CType>
bool EvalEPMeanProd(MultiV<CType>& vC,  EMVOpType eOPType, const MultiV<CType>& vA, const MultiV<CType>& vB, const Matrix<CType>& Cab)
{
...
template<class CType>
bool EvalEPCovMatProd(Matrix<CType>& Cuu, EMVOpType eOPType, const MultiV<CType>& vX, const MultiV<CType>& vY, const Matrix<CType>& Cxx, const Matrix<CType>& Cyy, const Matrix<CType>& Cxy)
{
...
template<class CType>
bool EvalEPCrossCovMatProd(Matrix<CType>& Cuz, EMVOpType eOPType, const MultiV<CType>& vX, const MultiV<CType>& vY, const Matrix<CType>& Cxz, const Matrix<CType>& Cyz)
{
	uint uGADim = vX.m_uGADim;
