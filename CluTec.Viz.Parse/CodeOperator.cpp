...
// file:      CodeOperator.cpp
//
// summary:   Implements the code operator class
//
...
bool CCodeOperator::Apply(CCodeBase* pCodeBase, SCodeData *pData)
{
	...
	if (m_bBinary)
	{
		if (!pCodeBase->Pop(pvL))
		{
			if (!m_bUnary)
			{
				pCodeBase->m_ErrorList.NoLRVal(iLine, iPos);
				return false;
			}
...
//				}
	...
	else // Create temporary variable
	{
		try
		{
			CCodeVar &rResVar = pBase->NewTempVar();
		...
		{
			pBase->m_ErrorList.GeneralError(xEx.PrintError().c_str(), iLine, iPos);
			return false;
		}
