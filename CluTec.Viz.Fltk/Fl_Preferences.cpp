//
// "$Id: Fl_Preferences.cxx,v 1.1 2004/02/18 16:10:28 chp Exp $"
//
...
#  define FLPREFS_RESOURCE	"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders"
  int appDataLen = int(strlen(vendor) + strlen(application) + 8);
  DWORD type, nn;
  LONG err;
  HKEY key;
  ...
  vendor_      = _strdup(vendor);
  application_ = _strdup(application);
...
{
  if ( path ) path_ = _strdup( path ); else path_ = 0;
...
void Fl_Preferences::Node::set( const char *name, const char *value )
{
  ...
  entry[ nEntry ].name = _strdup( name );
  entry[ nEntry ].value = value?_strdup( value ):0;
  lastEntrySet = nEntry;
  nEntry++;
  dirty_ = 1;
}
...
void Fl_Preferences::Node::add( const char *line )
{
  ...
  int a = (int)strlen( dst );
  int b = (int)strlen( line );
...
//
// End of "$Id: Fl_Preferences.cxx,v 1.1 2004/02/18 16:10:28 chp Exp $".
//
