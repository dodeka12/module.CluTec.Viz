...
int Fl::compose(int& del)
{
	...
	if (compose_state == 1)
	{
		...
		for (const char* p = compose_pairs; *p; p += 2)
			...
			{
				if (p[1] == ' ')
					e_text[0] = char((p-compose_pairs)/2)+char(0xA0); //TODO
		...
		if (e_length)
		{
			// compose key also "quotes" control characters
			compose_state = 0;
			return 1;
		}
	...
	else if (compose_state)
	{
		...
		for (const char* p = compose_pairs; *p; p += 2)
		{
			...
			{
				e_text[0] = char((p-compose_pairs)/2)+0xA0; // TODO:
