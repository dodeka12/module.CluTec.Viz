//
// "$Id: Fl_File_Chooser2.cxx,v 1.1 2004/02/18 16:10:24 chp Exp $"
//
...
//  puts("fileNameCB()");
...
#endif /* WIN32 || __EMX__ */
      directory(pathname);
    } else if ((type_ & CREATE) || _access(pathname, 0) == 0) {
           ...
           Fl::event_key() != FL_BackSpace)
  {
...
#if (defined(WIN32) && ! defined(__CYGWIN__)) || defined(__EMX__)
      if (_strnicmp(filename, file, min_match) == 0)
#else
...
#if (defined(WIN32) && ! defined(__CYGWIN__)) || defined(__EMX__)
	    if (_strnicmp(file, matchname, max_match) == 0)
#else
    ...
    if ((type_ & CREATE || _access(fileName->value(), 0) == 0) &&
...
Fl_File_Chooser::update_preview()
{
  ...
  if (!image) {
    ...
    for (ptr = preview_text_; *ptr && (isprint(int((unsigned char)*ptr)) || isspace(int((unsigned char)*ptr))); ptr ++);
...
//
// End of "$Id: Fl_File_Chooser2.cxx,v 1.1 2004/02/18 16:10:24 chp Exp $".
//
