//
// "$Id: Fl_Text_Display.cxx,v 1.1 2004/02/18 16:10:30 chp Exp $"
//
...
void Fl_Text_Display::draw_vline(int visLineNum, int leftClip, int rightClip,
	int leftCharIndex, int rightCharIndex) {
		...
		for ( i = 0; i < charLen; i++ ) {
			...
			if ( charStyle != style ) {
				draw_string( style, startX, Y, X, outStr, int(outPtr - outStr) ); // TODO: Check if warning is gone
				outPtr = outStr;
				startX = X;
				style = charStyle;
			}
		...
		draw_string( style, startX, Y, X, outStr, int(outPtr - outStr) );
...
int Fl_Text_Display::handle(int event) 
{
	...
	case FL_HIDE:
		if (active_r()) 
		{
			// Changed 19.1.2013; If not asking whether event occured inside text area, 
			// the cursor flickers on some machines.
			if (!Fl::event_inside(text_area.x, text_area.y, text_area.w, text_area.h))
				window()->cursor(FL_CURSOR_DEFAULT);
...
//
// End of "$Id: Fl_Text_Display.cxx,v 1.1 2004/02/18 16:10:30 chp Exp $".
//
