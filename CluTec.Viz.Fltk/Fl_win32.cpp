//
// "$Id: Fl_win32.cxx,v 1.2 2005/10/18 08:23:35 chp Exp $"
//
...
int fl_wait(double time_to_wait) {
  ...
  if (time_to_wait < 2147483.648) 
  {
	  ...
	  if (!have_message) 
	  {
		  ...
		  timerid = (int)SetTimer(NULL, 0, t, NULL);
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_MOUSELEAVE:
	  // Don't seem to need this. Ch. Perwass 22.03.2007
    Fl::belowmouse(0);
  ...
  case WM_SYSKEYUP:
    ...
    Fl::e_keysym = ms2fltk((int)wParam,(int)(lParam&(1<<24)));
...
////////////////////////////////////////////////////////////////
/// Get info of monitor extents at given rectangle

void Fl::GetMonitorExtentsAtRect( int iX, int iY, int iW, int iH, 
							int &iLeft, int &iRight, int &iTop, int &iBottom )
{
	RECT rectWnd;
	MONITORINFO infoMonitor;

	// Get Extents of monitor the window is to be displayed on
	infoMonitor.cbSize = sizeof( MONITORINFO );

	rectWnd.left = iX;
	rectWnd.top = iY;
	rectWnd.right = iX + iW - 1;
	rectWnd.bottom = iY + iH - 1;

	HMONITOR hMonitor = MonitorFromRect( &rectWnd, MONITOR_DEFAULTTONEAREST );
	GetMonitorInfo( hMonitor, &infoMonitor );

	iLeft = infoMonitor.rcWork.left;
	iRight = infoMonitor.rcWork.right;
	iBottom = infoMonitor.rcWork.bottom;
	iTop = infoMonitor.rcWork.top;
}
...
int Fl_X::fake_X_wm(const Fl_Window* w,int &X,int &Y, int &bt,int &bx, int &by) 
{
	...
	int ret = bx = by = bt = 0;
	int iScrWidth, iScrHeight, iScrLeft, iScrTop, iScrRight, iScrBottom;
	RECT rectWnd;
	MONITORINFO infoMonitor;
	...
	rectWnd.left = w->x();
	rectWnd.top = w->y();
	rectWnd.right = w->x() + w->w() - 1;
	rectWnd.bottom = w->y() + w->h() - 1;

	HMONITOR hMonitor = MonitorFromRect( &rectWnd, MONITOR_DEFAULTTONEAREST );
	GetMonitorInfo( hMonitor, &infoMonitor );

	iScrLeft = infoMonitor.rcWork.left;
	iScrRight = infoMonitor.rcWork.right;
	iScrBottom = infoMonitor.rcWork.bottom;
	iScrTop = infoMonitor.rcWork.top;
	iScrWidth = iScrRight - iScrLeft + 1;
	iScrHeight = iScrBottom - iScrTop + 1;
	...
	if (iScrRight < X+dx+ w->w()) 
		X = iScrRight - w->w() - dx;

	if (iScrBottom < Y+dy+ w->h()) 
		Y = iScrBottom - w->h() - dy;
...
void Fl_Window::resize(int X,int Y,int W,int H) {
  ...
  if (resize_from_program && shown()) {
	...
	i->lExStyle = (long) GetWindowLongPtr( (HWND) GetWindowHandle(), GWL_EXSTYLE );

	if ( (i->lExStyle & WS_EX_TOPMOST) )
	    SetWindowPos(i->xid, HWND_TOPMOST, X, Y, W, H, flags);
	else
...
Fl_X* Fl_X::make(Fl_Window* w, HWND hParent ) 
{
	...
	RegisterClassEx(&wc);
	if (!fl_wake_msg) 
		fl_wake_msg = RegisterWindowMessage(message_name);
...
#include "filename.H" // need so FL_EXPORT fl_filename_name works
...
void Fl_Window::show() 
{
	...
	if (!shown()) 
	{
		...
		Fl_X::make(this, (HWND) m_pvParentHandle );
	} 
	else 
	{
...
//
// End of "$Id: Fl_win32.cxx,v 1.2 2005/10/18 08:23:35 chp Exp $".
//
