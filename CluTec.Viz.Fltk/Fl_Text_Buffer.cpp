//
// "$Id: Fl_Text_Buffer.cxx,v 1.3 2004/04/16 08:01:29 chp Exp $"
//
...
int Fl_Text_Buffer::character_width( char c, int indent, int tabDist, char nullSubsChar ) {
  ...
  if ( c == '\t' )
    ...
    return int(strlen( ControlCodeTable[ c ] ) + 2);
  else if ( c == 127 )
    return 5;
  else if ( c == nullSubsChar )
    return 5;
...
int Fl_Text_Buffer::insert_( int pos, const char *s ) {
  int insertedLength = int(strlen( s ));
...
void Fl_Text_Buffer::insert_column_( int column, int startPos, const char *insText,
  ...
  *nInserted = int(outPtr - outStr);
  *nDeleted = end - start;
  *endPos = start + int ( outPtr - outStr ) - len + endOffset;
  free( (void *) outStr );
}
...
static void insertColInLine( const char *line, char *insLine, int column, int insWidth,
  ...
  if ( *insLine == '\0' && *linePtr == '\0' ) {
    *outLen = *endOffset = int(outPtr - outStr);
    return;
  }
  ...
  free( (void *) retabbedStr );
  *endOffset = int(outPtr - outStr);
  *outLen = int( outPtr - outStr ) + len;
}
...
static void overlayRectInLine( const char *line, char *insLine, int rectStart,
  ...
  *endOffset = int(outPtr - outStr);
  *outLen = (int)(( outPtr - outStr ) + strlen( linePtr ));
}
...
int Fl_Text_Buffer::findchar_backward( int startPos, char searchChar,
  ...
  // Changed startPos >= mLength to startPos > mLenght for this to
  // work if insert position is at the last position in the text buffer.
  // 3.4.04 by C. Perwass
  if ( startPos <= 0 || startPos > mLength ) {
    *foundPos = 0;
    return 0;
...
Fl_Text_Buffer::insertfile(const char *file, int pos, int buflen) {
  ...
  for (; (r = (int)fread(buffer, 1, buflen - 1, fp)) > 0; pos += r) {
    buffer[r] = (char)0;
    insert(pos, buffer);
  }
...
Fl_Text_Buffer::outputfile(const char *file, int start, int end, int buflen) {
  ...
  for (int n; (n = min(end - start, buflen)); start += n) {
    ...
    int r = int(fwrite(p, 1, n, fp));// TODO: explicit cast to int? 
    free((void *)p);
    if (r != n) break;
  }
...
//
// End of "$Id: Fl_Text_Buffer.cxx,v 1.3 2004/04/16 08:01:29 chp Exp $".
//
