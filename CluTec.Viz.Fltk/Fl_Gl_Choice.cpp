//
// "$Id: Fl_Gl_Choice.cxx,v 1.1 2004/02/18 16:10:25 chp Exp $"
//
...
#  endif
  ...
  for (int i = 1; ; i++) 
  {
	...
	if (!DescribePixelFormat(fl_gc, i, sizeof(pfd), &pfd)) 
		break;
    ...
    if (pfd.iPixelType != ((m&FL_INDEX)?PFD_TYPE_COLORINDEX:PFD_TYPE_RGBA)) 
		continue;

    if ((m & FL_ALPHA) && !pfd.cAlphaBits) 
		continue;

    if ((m & FL_ACCUM) && !pfd.cAccumBits) 
		continue;

    if ((!(m & FL_DOUBLE)) != (!(pfd.dwFlags & PFD_DOUBLEBUFFER))) 
		continue;

    if ((!(m & FL_STEREO)) != (!(pfd.dwFlags & PFD_STEREO))) 
		continue;

    if ((m & FL_DEPTH) && !pfd.cDepthBits) 
		continue;

    if ((m & FL_STENCIL) && !pfd.cStencilBits) 
		continue;
...
//
// End of "$Id: Fl_Gl_Choice.cxx,v 1.1 2004/02/18 16:10:25 chp Exp $".
//
