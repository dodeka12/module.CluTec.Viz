//
// "$Id: fl_set_fonts_win32.cxx,v 1.1 2004/02/18 16:10:41 chp Exp $"
//
...
enumcb(CONST LOGFONT    *lpelf,
  ...
  strcpy(buffer+1, n);
  buffer[0] = ' '; Fl::set_font((Fl_Font)(fl_free_font++), _strdup(buffer));
  if (lpelf->lfWeight <= 400)
    buffer[0] = 'B', Fl::set_font((Fl_Font)(fl_free_font++), _strdup(buffer));
  buffer[0] = 'I'; Fl::set_font((Fl_Font)(fl_free_font++), _strdup(buffer));
  if (lpelf->lfWeight <= 400)
    buffer[0] = 'P', Fl::set_font((Fl_Font)(fl_free_font++), _strdup(buffer));
  return 1;
}
...
//
// End of "$Id: fl_set_fonts_win32.cxx,v 1.1 2004/02/18 16:10:41 chp Exp $".
//
