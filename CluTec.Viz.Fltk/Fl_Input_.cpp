//
// "$Id: Fl_Input_.cxx,v 1.2 2005/07/13 05:46:25 chp Exp $"
//
...
double Fl_Input_::expandpos(
  ...
  int n = 0;
  if (input_type()==FL_SECRET_INPUT) n = int(e-p);
  else while (p<e) {
    int c = *p++ & 255;
    if (c < ' ' || c == 127) {
...
int Fl_Input_::line_end(int i) const {
  ...
  if (wrap()) {
    ...
    for (const char* p=value()+j; ;) {
      char buf[MAXBUF];
      p = expand(p, buf);
      if (p-value() >= i) return (int)(p-value());
      p++;
    }
  } else {
...
int Fl_Input_::line_start(int i) const {
  ...
  if (wrap()) {
    ...
    for (const char* p=value()+j; ;) {
      ...
      if (e-value() >= i) return (int)(p-value());
      p = e+1;
    }
  } else return j;
}
...
//
// End of "$Id: Fl_Input_.cxx,v 1.2 2005/07/13 05:46:25 chp Exp $".
//
