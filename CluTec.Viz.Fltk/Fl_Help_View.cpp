//
// "$Id: Fl_Help_View.cxx,v 1.6 2005/05/30 08:00:59 chp Exp $"
//
...
Fl_Help_View::add_block(const char* s,		// I - Pointer to start of block text
		...
		uchar font, uchar fsize, unsigned char border)	// I - Draw border?
{
...
void Fl_Help_View::draw()
{
	...
	* attrs;		// Pointer to start of element attributes
	...
	fl_push_clip(
			x() + Fl::box_dx(b),	// + m_iLeftMargin,
			y() + Fl::box_dy(b) + m_iTopMargin,
			ww - Fl::box_dw(b) /*- m_iLeftMargin*/ - m_iRightMargin,
			hh - Fl::box_dh(b) - m_iTopMargin - m_iBottomMargin);
	fl_color(textcolor_);
	...
	for (i = 0, block = blocks_; i < nblocks_; i++, block++)
	{
		if (((block->y + block->h) >= topline_) && (block->y < (topline_ + h())))
		{
			...
			{
				if (((*ptr == '<') || isspace((unsigned char) *ptr)) && (s > buf))
				{
					if (!head && !pre)
					{
						sw = 0;
						...
						*s = '\0';
						...
						if (((xx + ww + sw) > block->w) && (xx > block->x))
						{
							if (line < 31)
							{
								line++;
							...
							// Draw overbar up to here already
							if ((iOBarStart >= 0) && (xx > iOBarStart))
							{
								int xo = x() + m_iLeftMargin;
								int yo = yy + y() + m_iTopMargin;
							...
							fl_draw(buf,
									xx + x() + m_iLeftMargin - leftline_,
									yy + y() + m_iTopMargin);

							xx += ww;
						}
						else if (((xx + ww + sw) > block->w) && (xx == block->x))
						{
							fl_draw(buf,
									xx + sw + x() + m_iLeftMargin - leftline_,
									yy + y() + m_iTopMargin);
				...
				if (*ptr == '<')
				{
					...
					*s = '\0';
					...
					{
						fl_line(
								block->x + x() + m_iLeftMargin,
								(int) (yy - 1.5 * fsize + y() + m_iTopMargin),
								block->w + x(),
								(int) (yy - 1.5 * fsize + y()));
						 ...
						 (strcasecmp(buf, "PRE") == 0))
					{
						...
						if (strcasecmp(buf, "LI") == 0)
						{
							...
							fl_draw("\267",
									xx - fsize + x() + m_iLeftMargin - leftline_,
									yy + y() + m_iTopMargin);
						}
					...
					else if (strcasecmp(buf, "FONT") == 0)
					{
						...
						{
							iNewFSize = (uchar) atoi(attr);
							if ((iNewFSize <= 5) && (iNewFSize > 50))
							{
								iNewFSize = fsize;
							}
						}
			...
			*s = '\0';
			...
			if ((s > buf) && !pre && !head)
			{
				...
				if (((xx + sw + ww) > block->w) && (xx > block->x))
				{
					if (line < 31)
					{
						line++;
				...
				else if (((xx + sw + ww) > block->w) && (xx == block->x))
				{
					bNewLine = true;
				}
			}
...
void Fl_Help_View::format()
{
	...
	* attrs;		// Pointer to start of element attributes
	...
	while (!done)
	{
		...
		border       = 0;
		hh           = 0;
		block        = add_block(value_, xx, yy, hsize_, 0, font, fsize);
		row          = 0;
		head         = 0;
		pre          = 0;
		talign       = LEFT;
				...
				*s = '\0';
				...
				if (!head && !pre)
				{
					...
					else
					{
						if (linkdest[0])
						{
							add_link(linkdest, xx + sw, yy - fsize, ww, fsize);
						}
			...
			if (*ptr == '<')
			{
				...
				*s = '\0';
				...
				else if (strcasecmp(buf, "BODY") == 0)
				{
					...
					if (get_attr(attrs, "SIZE", attr, sizeof(attr)) != NULL)
					{
						int iNewFSize = (uchar) atoi(attr);
						if ((iNewFSize > 5) && (iNewFSize < 50))
						{
							textsize_ = iNewFSize;
							pushfont(font, textsize_);
						}
					}
					else
					{
...
*/
					 ...
					 (strcasecmp(buf, "TABLE") == 0))
				{
					...
					else if ((tolower(buf[0]) == 'h') && isdigit(buf[1]))
					{
						font  = FL_HELVETICA_BOLD;
						fsize = (uchar) (textsize_ + '7' - buf[1]);
					}
					...
					if (row)
					{
						block = add_block(start, xx, yy, block->w, 0, font, fsize);
					}
					else
					{
						block = add_block(start, xx, yy, hsize_, 0, font, fsize);
					}
					 ...
					 (strcasecmp(buf, "/TABLE") == 0))
				{
					...
					if (row)
					{
						block = add_block(ptr, xx, yy, block->w, 0, font, fsize);
					}
					else
					{
						block = add_block(ptr, xx, yy, hsize_, 0, font, fsize);
					}
...
//						font = textfont_;
					...
					yy        = blocks_[row].y;
					hh        = 0;
					block     = add_block(start, xx, yy, xx + ww, 0, font, fsize, border);
					needspace = 0;
					line      = 0;
		...
		// Check whether buf contains a string and we are neither
		// in pre or the head
		if ((s > buf) && !pre && !head)
		{
			*s = '\0';
	...
	else
	{
		...
		if ((size_ < (h() - 8)) || (h() <= 20) || (w() <= 20)) { scrollbar_.hide(); }
		else
		{
...
void Fl_Help_View::format_table(int* table_width,	// O - Total table width
		...
		const char* table)					// I - Pointer to start of table
{
	...
	*table_width = 0;
	...
	{
		if (((*ptr == '<') || isspace((unsigned char) *ptr)) && (s > buf) && incell)
		{
			// Check width...
			if (needspace)
			{
		...
		if (*ptr == '<')
		{
			...
			for (s = buf, ptr++; *ptr && *ptr != '>' && !isspace((unsigned char) *ptr); )
			{
				if (s < (buf + sizeof(buf) - 1))
				{
					*s++ = *ptr++;
...
/////////////////////////////////////////
// Added by CP for testing
	//*table_width = width + num_columns*6;
	//return;
////////////////////////////////////////
...
Fl_Shared_Image* Fl_Help_View::get_image(const char* name, int W, int H)
{
	...
	* tempptr;			// Pointer into temporary name
	...
	if ((strchr(directory_, ':') != NULL) && (strchr(name, ':') == NULL))
	{
		...
		if (link_) { localname = (*link_)(this, temp, linkdata_); }
		else{ localname = temp; }
	}
	...
	{
		localname = (*link_)(this, name, linkdata_);
	}
	else{ localname = name; }
...
int Fl_Help_View::get_length(const char* l)		// I - Value
{
...
Fl_Help_View::handle(int event)	// I - Event to handle
{
	...
	case FL_DRAG:
	case FL_MOVE:
		xx = Fl::event_x() - x() - m_iLeftMargin + leftline_;
		yy = Fl::event_y() - y() - m_iTopMargin + topline_;
		break;
	...
	hscrollbar_(xx, yy + hh - 17, ww - 17, 17)
{
	...
	linkcolor_ = FL_BLUE;	//FL_SELECTION_COLOR;
	textfont_  = FL_TIMES;
	textsize_  = 12;
	value_     = NULL;
...
Fl_Help_View::load(const char* f)	// I - Filename to load (may also have target)
{
	...
	if (link_)
	{
		localname = (*link_)(this, newname, linkdata_);
	}
	else
	{
		localname = filename_;
...
void Fl_Help_View::topline(const char* n)	// I - Target name
{
...
void Fl_Help_View::topline(int t)	// I - Top line number
{
	if (!value_)
	{
		return;
...
void Fl_Help_View::leftline(int l)	// I - Left position
{
	if (!value_)
	{
		return;
...
void Fl_Help_View::value(const char* v)		// I - Text to view
{
	if (!v)
	{
		return;
...
//
// End of "$Id: Fl_Help_View.cxx,v 1.6 2005/05/30 08:00:59 chp Exp $".
//
