//
// "$Id: Fl_File_Icon2.cxx,v 1.1 2004/02/18 16:10:25 chp Exp $"
//
...
Fl_File_Icon::load_image(const char* ifile)	// I - File to read from
{
	...
	else
	{
		...
		* const* ptr;			// Pointer into data array
		...
		colors = new Fl_Color[1i64 << (chars_per_color * 8)];
...
void Fl_File_Icon::load_system_icons(void)
{
	...
	if (!init)
	{
		...
		if (!kdedir)
		{
			...
			{
				if (!_access("/opt/kde", F_OK))
				{
					kdedir = "/opt/kde";
				}
				else if (!_access("/usr/local/share/mimelnk", F_OK))
				{
					kdedir = "/usr/local";
				}
				else
		...
		else if (!_access("/usr/share/icons/folder.xpm", F_OK))
		{
			// Load GNOME icons...
			icon = new Fl_File_Icon("*", Fl_File_Icon::PLAIN);
		...
		else if (!_access("/usr/dt/appconfig/icons", F_OK))
		{
			// Load CDE icons...
			icon = new Fl_File_Icon("*", Fl_File_Icon::PLAIN);
		...
		else if (!_access("/usr/lib/filetype", F_OK))
		{
			// Load SGI icons...
			icon = new Fl_File_Icon("*", Fl_File_Icon::PLAIN);
			...
			if (!_access("/usr/lib/filetype/install/iconlib/acroread.doc.fti", F_OK))
			{
			...
			if (!_access("/usr/lib/filetype/install/iconlib/html.fti", F_OK))
			{
			...
			if (!_access("/usr/lib/filetype/install/iconlib/color.ps.idle.fti", F_OK))
			{
				icon = new Fl_File_Icon("*.ppd", Fl_File_Icon::PLAIN);
		...
		if (iconfilename[0] && (pattern[0] || (strncmp(mimetype, "inode/", 6) == 0)))
		{
			...
			if (!_access(tmp, F_OK))
			{
				...
				for (i = 0; i < (int) (sizeof(paths) / sizeof(paths[0])); i++)
				{
					...
					if (!_access(full_iconfilename, F_OK)) { break; }
				}
			...
			else
			{
				...
				if (_access(full_iconfilename, F_OK)) { return; }
			}
...
//
// End of "$Id: Fl_File_Icon2.cxx,v 1.1 2004/02/18 16:10:25 chp Exp $".
//
