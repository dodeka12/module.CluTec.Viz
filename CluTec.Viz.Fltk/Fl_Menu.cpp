						...
						int selected) const 
{
	...
	if (selected) 
	{
		...
		if (fl_contrast(r,color)!=r) 
		{ 
			// back compatability boxtypes
			if (selected == 2) 
			{ 
				// menu title
				r = color;
	...
	if (flags & (FL_MENU_TOGGLE|FL_MENU_RADIO)) 
	{
		...
		{
			fl_draw_box(FL_FLAT_BOX, x+2, y+d, W, W, FL_LIGHT1);
			if (value()) 
			{
...
/*
				int tW = (W - Fl::box_dw(FL_OFLAT_BOX)) / 2 + 1;
				if ((W - tW) & 1) tW++;	// Make sure difference is even to center
				int td = Fl::box_dx(FL_OFLAT_BOX) + 1;
				td += 2;
...
*/
		...
		{
			fl_draw_box(FL_FLAT_BOX, x+2, y+d, W, W, FL_BACKGROUND2_COLOR);
			if (value()) 
			{
				fl_color(labelcolor_);
				int tx = x + 5;
...
{
	int left_edge = 0;
	int bottom_edge = Fl::h(), top_edge = 0;

#ifdef WIN32

	Fl::GetMonitorExtentsAtRect( X, Y, Wp, Hp, left_edge, right_edge, top_edge, bottom_edge );
	...
	if (m) 
	{
		for (; m->text; m = m->next()) 
		{
			...
			if (hh+LEADING>itemheight) 
				itemheight = hh+LEADING;
			...
			if (m->labelcolor_ || Fl::scheme() || m->labeltype_ > FL_NO_LABEL) 
				clear_overlay();
		}
	}
