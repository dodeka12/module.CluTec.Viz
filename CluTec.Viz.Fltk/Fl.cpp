//
// "$Id: Fl.cxx,v 1.2 2004/07/05 10:51:19 chp Exp $"
//
...
/*
	if ( fl_xmousewin )
	{
		mx = Fl::e_x_root - fl_xmousewin->x() - xx;
		my = Fl::e_y_root - fl_xmousewin->y() - yy;
	}
	else
	{
*/
...
/*
	int iX = 0, iY = 0;
	Fl_Widget *pWidget = (Fl_Widget *) o;

	while ( pWidget->parent() )
	{
		iX += pWidget->x();
		iY += pWidget->y();
...
int Fl::handle(int e, Fl_Window* window)
{
	...
	case FL_SHORTCUT:
		...
		/* Don't want this. (C.Perwass 4.7.04)
	...
	case FL_MOUSEWHEEL:
		...
		// Try it as keystroke, sending it to focus and all parents:
		for (wi = grab() ? grab() : focus(); wi; wi = wi->parent())
			if (send(FL_MOUSEWHEEL, wi, window)) 
				return 1;
...
//
// End of "$Id: Fl.cxx,v 1.2 2004/07/05 10:51:19 chp Exp $".
//
