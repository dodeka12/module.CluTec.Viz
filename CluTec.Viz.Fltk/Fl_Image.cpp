//
// "$Id: Fl_Image.cxx,v 1.1 2004/02/18 16:10:26 chp Exp $"
//
...
Fl_RGB_Image::Fl_RGB_Image(const uchar *bits, int W, int H, int D, int LD) :  Fl_Image(W,H,D)
{
	id = 0;
	mask = 0;

	unsigned uSize = unsigned(W) * unsigned(H) * unsigned(D);

	pArray = new uchar[ uSize ];
	alloc_array = 1;

	memcpy( pArray, bits, uSize );

	data((const char **)&pArray, 1); 
...
Fl_RGB_Image::~Fl_RGB_Image() {
  uncache();
  if (alloc_array) delete[] (uchar *)pArray;
}
...
Fl_Image *Fl_RGB_Image::copy(int W, int H) {
  ...
  if ((W == w() && H == h()) ||
      !w() || !h() || !d() || !pArray) {
    return new Fl_RGB_Image(pArray, w(), h(), d(), ld());
  }
  if (W <= 0 || H <= 0) return 0;

  // OK, need to resize the image data; allocate memory and 
  Fl_RGB_Image	*new_image;	// New RGB image
  uchar		*new_array,	// New pArray for image data
		*new_ptr;	// Pointer into new pArray
  const uchar	*old_ptr;	// Pointer into old pArray
  ...
  for (dy = H, sy = 0, yerr = H, new_ptr = new_array; dy > 0; dy --) {
    for (dx = W, xerr = W, old_ptr = pArray + sy * (w() * d() + ld());
	 dx > 0;
	 dx --) {
...
void Fl_RGB_Image::color_average(Fl_Color c, float i) {
  ...
  if (d() < 3) {
    ...
    for (new_ptr = new_array, old_ptr = pArray, y = 0; y < h(); y ++, old_ptr += ld())
      for (x = 0; x < w(); x ++) {
...
void Fl_RGB_Image::desaturate() {
  ...
  // Free the old pArray as needed, and then set the new pointers/values...
  if (alloc_array) delete[] (uchar *)pArray;
...
void Fl_RGB_Image::draw(int XP, int YP, int WP, int HP, int cx, int cy) {
  ...
  if (!id) {
    ...
    fl_draw_image(pArray, 0, 0, w(), h(), d(), ld());
    fl_end_offscreen();

    if (d() == 2 || d() == 4) {
      mask = fl_create_alphamask(w(), h(), d(), ld(), pArray);
    }
  }
#ifdef WIN32
  if (mask) {
...
//
// End of "$Id: Fl_Image.cxx,v 1.1 2004/02/18 16:10:26 chp Exp $".
//
