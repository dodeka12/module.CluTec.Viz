...
// file:      XMLdom.cpp
//
// summary:   Implements the xm ldom class
//
...
#include "CluTec.Viz.Base\CLUDebug.h"
#include "XMLdom.h"
	...
	throw (CCluException)
{
	...
	if (!(m_pDoc = xmlParseMemory(pcMemory, iSize))) //TODO 
	{
...
////////////////////////////////////////////////////
/// Write XML Tree to memory

void CXMLdom::WriteMemory(vector<char> &vecMemory) 
	throw (CCluException)
{
	if (!m_pDoc)
		return;
	...
	xmlDocDumpFormatMemory(m_pDoc, &pcData, &iDataLen, 1);

	vecMemory.resize( iDataLen );
	memcpy( &vecMemory[0], pcData, iDataLen );
...
bool CXMLdom::GetElementTree(CXMLElement &rEl, bool bFromTop)
{
	string sElName, sElContent, sVal;
	map<string, string> mapProp;
	CXMLElement XMLEl;

	CXMLTree &rTree = rEl.GetSubTree();
	...
	do
	{
		...
		// Elements with name "text" are content elements
		// We neglect the order of these with respect to other embedded elements
		// and simply collect all "text" elements that do not only contain spaces and tabs 
		// in a string separated by '\n'.
		if (sElName == "text")
		{
			if ( GetContent( sVal ) )
			{
				std::string::size_type nStartIdx, nEndIdx;
				if ( (nStartIdx = sVal.find_first_not_of( " \t\n\r" )) != std::string::npos )
				{
					nEndIdx = sVal.find_last_not_of( " \t\n\r" );
					sVal = sVal.substr( nStartIdx, nEndIdx - nStartIdx + 1 );
		...
		else
		{
			...
			// Content of element is already contained in "text" elements
			//// Get content of element
			//if (!HasChildElements())
			//	GetContent(sElContent);

			// Initialize CXMLElement instance
			XMLEl.Set(sElName.c_str(), "", mapProp);
	...
	// Write content collected
	rEl.SetContent( sElContent );
